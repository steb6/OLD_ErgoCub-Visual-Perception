...
def main(runtime):
    ...
    data_loader = DataSet(iterations=22)

    if runtime == 'trt':
        ####  Setup TensorRT Engine
        # backbone = Infer('./segmentation/tensorrt/assets/seg_linux.engine')
        ####  Run Evaluation
        for i, x in tqdm.tqdm(enumerate(data_loader)):
            # res = backbone(x['input'])
            # categories = cv2.resize(res, dsize=(640, 480), interpolation=cv2.INTER_NEAREST)
            categories = np.array(Image.open(f'./segmentation/tensorrt/assets/segmentations/pt_mask_wsl{i}.png'))
            ...
            cv2.waitKey(0)
            # Image.fromarray(categories).save(f'./segmentation/tensorrt/assets/segmentations/pt_mask_wsl{i}.png')
